...
public class CheckoutOnePageRefreshAction extends BaseAction
{
    ...
    public String execute()
    {
        ...
        try
        {
            ...
            if (billingAddrId != null)
            {
                ...
                if (pdArray != null && pdArray.length > 0)
                {
                    ...
                    if (!added)
                    {
                        ...
                        code = pdArray[0].getCode() + ((pdArray[0].getSubCode() == null) ? ""
                                : ("~~" + pdArray[0].getSubCode()));
            ...
            {
                String msg = "checkoutOrder before populateCheckoutOrderWithOrderTotals : "
                        + checkoutOrder.getLifecycleId() + " with ";
            ...
            if (log.isDebugEnabled())
            {
                String msg = "checkoutOrder after populateCheckoutOrderWithOrderTotals : "
                        + checkoutOrder.getLifecycleId() + " with ";
                if (checkoutOrder.getOrderTotals() == null)
                {
                    msg += " NULL OrderTotals";
            ...
            if (log.isDebugEnabled())
            {
                log.debug("checkoutMsg = " + checkoutMsg);
            }
            ...
            if (isMultiVendorMode)
            {
                ...
                for (int i = 0; i < checkoutOrder.getVendorOrders().length; i++)
                {
                    ...
                    if (order.getShippingQuote() != null)
                    {
                        ...
                        quote.setFormattedTotalExTax(
                                kkAppEng.formatPrice(order.getShippingQuote().getTotalExTax()));
                        quote.setFormattedTotalIncTax(
                                kkAppEng.formatPrice(order.getShippingQuote().getTotalIncTax()));
            ...
            formattedDeliveryAddr = kkAppEng
                    .removeCData(checkoutOrder.getDeliveryFormattedAddress());
            ...
            throws KKAppException
    {
        ...
        for (int i = 0; i < opArray.length; i++)
        {
            ...
            opClone.setFormattedTaxRate(
                    (op.getTaxRate().setScale(1, java.math.BigDecimal.ROUND_HALF_UP))
                            .toPlainString());
            opClone.setName(op.getName());
            opClone.setQuantity(op.getQuantity());
            opClone.setProductId(op.getProductId());
            String imgSrc = kkAppEng.getProdImage(op.getProduct(), op.getOpts(),
                    KKAppEng.IMAGE_TINY);
            opClone.setProdImgSrc(imgSrc);
            ...
            if (op.getOpts() != null && op.getOpts().length > 0)
            {
                ...
                for (int j = 0; j < op.getOpts().length; j++)
                {
                    ...
                    {
                        optClone.setFormattedCustPrice(
